...
{
    this->colaPrioridadNodos.push(*nodoRaiz);
...
void CostoUniforme::crearHijos(Nodo nodoCabeza)
{
...
void CostoUniforme::crearNodo(int posIHijo, int posJHijo, Nodo nodoCabeza)
{
    // Limites del entorno para crear nodos hijos
    int altoEntorno = this->miEntorno.getAlto();
    int anchoEntorno = this->miEntorno.getAncho();
...
estados CostoUniforme::toEstado(int itemEntorno)
{
    estados estado;
    switch (itemEntorno)
    {
