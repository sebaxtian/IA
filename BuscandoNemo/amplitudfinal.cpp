...
AmplitudFinal::AmplitudFinal()
{
    this->nodosCreados = 0;
    this->nodosExpandidos = 0;
}

AmplitudFinal::AmplitudFinal(NodoFinal * nodoRaiz, Entorno entorno)
{
...
NodoFinal * AmplitudFinal::busquedaPreferente()
{
...
void AmplitudFinal::crearHijos(NodoFinal nodoCabeza)
{
...
void AmplitudFinal::crearNodo(int posIHijo, int posJHijo, NodoFinal nodoCabeza)
{
    ...
    if(estadoHijo != onRoca)
    {
        ...
        //NodoFinal nodoHijo(camino,profundidad,costoAcumulado,flagObjetivos,posIHijo,posJHijo);
        //NodoFinal nodoHijo(posIHijo, posJHijo, estadoHijo, nodoCabeza.getEntorno());
        NodoFinal nodoHijo(posIHijo, posJHijo, estadoHijo);
...
estados AmplitudFinal::toEstado(int itemEntorno)
{
    estados estado;
    switch (itemEntorno)
    {
...
/*
int AmplitudFinal::validaObjetivo(int flagObjetivos, estados estado)
{
    if(estado == onNemo && flagObjetivos == 0)
    {
        flagObjetivos++;
    }
    if(estado == onMarlin && flagObjetivos == 1)
    {
        flagObjetivos++;
    }
    if(estado == onDori && flagObjetivos == 2)
    {
        flagObjetivos++;
    }
